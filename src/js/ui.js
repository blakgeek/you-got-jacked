...
function UI(config) {
	...
	var SUIT_MAPPING = {d: 'diamond', c: 'club', h: 'heart', s: 'spade'},
		...
		pressEvent = ("ontouchstart" in document.documentElement) ? 'touchstart' : 'click',
		self = this;

	$discardPile.on(pressEvent, function() {
	...
	$('.play-online').on(pressEvent, function() {
	...
	$home.find('.new-game').on(pressEvent, function() {

		setupWizard.reset();
		$body.removeClass(allModeClasses).addClass('wizard');
	});

	$dialog.find('.replay').on(pressEvent, function() {

		$body.removeClass(allModeClasses);
		jax.startOfflineGame(setupWizard.values());
	});

	$dialog.find('.new-game').on(pressEvent, function() {

		$body.removeClass(allModeClasses).addClass('wizard');
	});

	$wizard.find('.play').on(pressEvent, function() {

		$body.removeClass(allModeClasses);
		jax.startOfflineGame(setupWizard.values());
	});

	$home.find('.rules').on(pressEvent, function() {
	...
	$game.find('nav .rules').on(pressEvent, function() {
	...
	$game.find('nav .quit').on(pressEvent, function() {

		$body.removeClass(allModeClasses).addClass('home');
		$hand.off();
	});

	$tutorial.find('.back').on(pressEvent, function() {

		$body.removeClass(allModeClasses).addClass('game');
	});

	$tutorial.find('.new-game').on(pressEvent, function() {

		$body.removeClass(allModeClasses).addClass('wizard');
	});

	$board.on(pressEvent, 'li', function() {
	...
	for(name in Jax.BOARDS) {
		...
		board.forEach(cellFn);

		html.push('</ul>');
	}

	$('.board-picker .boards').html(html.join('')).on(pressEvent, 'ul', function() {

		$('.board-picker ul').removeClass('selected');
		$(this).addClass('selected');
	});

	$('.player-picker').on(pressEvent, 'li', function() {

		$('.player-picker li').removeClass('selected');
		$(this).addClass('selected');
	});

	$('.level-picker').on(pressEvent, 'li', function() {
	...
	function SetupWizard() {
		...
		$wizard.find('.next').on(pressEvent, function next() {
		...
		$wizard.find('.prev').on(pressEvent, function next() {
	...
	this.enableGameEvents = function() {
		...
		$jax.on({
			...
			gamestarted: function(event, data) {
				...
				$hand.on(pressEvent, 'li', function() {
					var $this = $(this);
					if($this.is('.active')) {
